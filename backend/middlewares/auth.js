...
const protect = async (req, res, next) => {
  try {
    ...
    if (!decoded.tenantDb) {
      res.status(401);
      throw new Error('Access denied: Tenant database mapping is missing in token scope');
    }
...
const restrictTo = (...roles) => {
  return (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      res.status(403);
      return next(new Error(`Permission denied: Access restricted to [${roles.join(', ')}]`));
    }
